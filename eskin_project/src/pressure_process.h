...
#define NUM_MIDI_CHANNELS 16 // MIDI通道数
...
class PressToMIDI{//将压力信号魔法般地变成midi信号
  ...
  void setConfig(const KeyConfig& cfg);//更换配置，重置所有按键状态
  //==========不同的乐器按键=========================================================================
  ...
  void _banKeys(bool ban, int row, int col,int u=1, int d=1, int l=1, int r=1);//u=up d=down l=left r=right 用来选择范围，标记或者解除标记某传感器为sensor类
  bool _weightBias( float& x, float& y, int row, int col,float& meanF,int u=1, int d=1, int l=1, int r=1);
  // 私有拷贝函数(缓存功能用的)
